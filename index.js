...
function keysAndValues(obj) {
  if (typeof obj === "object") {
    ...
    let valuesArr = Array.from(Object.values(obj));
...
function filterObjectsByKey(arr, key) {
  if (Array.isArray(arr)) {
    let filteredArr = arr.filter((value) => value.hasOwnProperty(key));
    return filteredArr;
  } else {
    return [];
  }
...
function getObjectValuesSum(obj) {
  if (typeof obj === "object") {
    ...
    return nums.reduce((prevValue, curValue) => prevValue + curValue);
  } else {
    return 0;
  }
